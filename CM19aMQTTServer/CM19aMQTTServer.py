...
class CM19aMQTTServer:
    def __init__():
    	self.log = startLogging(LOGFILE)

	#global cm19a, log, server, mqtt_alive
	self.config_paths = [
	    "/etc/CM19aMQTTServer.ini", 
	    os.path.dirname(os.path.realpath(__file__)) + '/CM19aProtocol.ini'
	]
...
class CM19aDevice(threading.Thread):
    ...
    def __init__(self, refresh=1, loginstance=None, polling=False):
        ...
        if loginstance:
            ...
            self.log = start_logging("CM19a_X10_USB", "./CM19a.log")
...
def startLogging(progname="CM19a_X10_USB", logfile='./cm19a.log'):
    return start_logging(progname, logfile)
...
def startMQTT( client, host, port ):
    ...
    print "Connecting to MQTT server at: %s:%s" % (MQTT_HOST, MQTT_PORT)
